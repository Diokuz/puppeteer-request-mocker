...
it('skipAllQuqeryParamsExcept does not affects output name', () => {
  const m = 'GET'
  const p = ''
  const skipAllQueryParamsExcept = ['foo']
  const name1 = mfn('http://example.com?foo=bar&x=y&y=x', m, p, [], skipAllQueryParamsExcept)
  const name2 = mfn('http://example.com?x=y&foo=bar', m, p, [], skipAllQueryParamsExcept)
  expect(name1).toBe(name2)
})

it('skipAllQuqeryParamsExcept > 1 does not affects output name', () => {
  const m = 'GET'
  const p = ''
  const skipAllQueryParamsExcept = ['foo', 'bar']
  const name1 = mfn('http://example.com?foo=bar&bar=foo&x=y&y=x', m, p, [], skipAllQueryParamsExcept)
  const name2 = mfn('http://example.com?bar=foo&x=y&foo=bar', m, p, [], skipAllQueryParamsExcept)
  expect(name1).toBe(name2)
})
