...
function isLookedRequest(req, params) {
  return req._url === params.url && (params.method ? req._method === params.method : true)
}
...
class Mocker {
  ...
  requestChecker(params) {
    return request => {
      params.forEach(item => {
        if (isLookedRequest(request, item)) {
          this.cachedReqs.push(request)
        }
      })
    }
  }

  waitForRequests(reqParams) {
    this.page.on('request', this.requestChecker(reqParams))
  }
