...
module.exports = function createHandler (params) {
  const { reqSet, workDir, mockList, okList, verbose, force, ci, skipQueryParams, skipAllQueryParamsExcept, skipPostParams } = params
  ...
  return function handlerResponse(interceptedResponse) {
    ...
    const fn = storage.name(url, method, postData, workDir, skipQueryParams, skipAllQueryParamsExcept, skipPostParams)
    ...
    interceptedResponse.text()
      .then((text) => {
        ...
        return storage.write({
          ...
          logger(`Fail to save the file because of `, err)
